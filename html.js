...
function defaultPostage() {
  return section("Postage", `<ul><li>Fast UK dispatch.</li><li>Tracked delivery on most items.</li></ul>`);
}
function defaultReturns() {
  return section("Returns", `<ul><li>30-day returns accepted.</li></ul>`);
}
function buildBitznBobzHtml(payload) {
  const {
    seoTitle, condition, shortDesc,
    featuresHtml, specsHtml, whatsInBoxHtml,
    postageHtml, returnsHtml
  } = payload;
  return `
<div style="font-family:Aptos,Arial,sans-serif;background:#000;color:#fff;padding:14px;border:3px solid #FFD400;border-radius:12px;">
  <div style="font-size:22px;font-weight:900;color:#FFD400;margin-bottom:8px;">BITZ’n’BOBZ</div>
