...
export default function App() {
  ...
  const generateAll = async () => {
    ...
    try {
      const { file, error } = await callGenerateAll(urls);

      if (error) throw new Error(error);
      if (!file) throw new Error("No file returned from generator.");
      ...
      <div style={{
        ...
        <footer style={{
          ...
          <div style={{ fontSize: 12, fontWeight: 800 }}>
            Thanks for choosing BITZ’n’BOBZ — listings built fast and clean.
          </div>
        </footer>
      </div>
    </div>
